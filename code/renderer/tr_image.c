...
//glclamp
	tr.dlightImage = R_CreateImage("*dlight", (byte *)data, DLIGHT_SIZE, DLIGHT_SIZE, qfalse, qfalse, r_glClampToEdge->integer ? GL_CLAMP_TO_EDGE : GL_CLAMP );
//-glclamp
}
...
static void R_CreateFogImage( void ) {
	...
	// what we want.
//glclamp
	tr.fogImage = R_CreateImage("*fog", (byte *)data, FOG_S, FOG_T, qfalse, qfalse, r_glClampToEdge->integer ? GL_CLAMP_TO_EDGE : GL_CLAMP );
//-glclamp
...
//glclamp
		tr.scratchImage[x] = R_CreateImage("*scratch", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE, qfalse, qtrue, r_glClampToEdge->integer ? GL_CLAMP_TO_EDGE : GL_CLAMP );
//-glclamp
	}
...
qhandle_t RE_RegisterSkin( const char *name ) {
	...
	if ( strcmp( name + strlen( name ) - 5, ".skin" ) ) {
		...
		skin->surfaces[0]->shader = R_FindShader( name, LIGHTMAP_NONE, qtrue, qfalse );
		return hSkin;
	}
