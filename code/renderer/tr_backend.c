...
//glclamp
		qglTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, r_glClampToEdge->integer ? GL_CLAMP_TO_EDGE : GL_CLAMP );
		qglTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, r_glClampToEdge->integer ? GL_CLAMP_TO_EDGE : GL_CLAMP );	
//-glclamp
	} else {
		if (dirty) {
...
#ifdef USE_RENDERER_GLSLFBO
			&& (!fbo || (fbo && tr.usingFrameBufferObject && tr.usingFboStencil))
#endif //USE_RENDERER_GLSLFBO
			) {
		int i;
		long sum = 0;
