...
static	void R_ColorShiftLightingBytes( byte in[4], byte out[4] ) {
	...
	cap = r_mapOverBrightCap->integer;
	if (cap > 255) {
		cap = 255;
	} else if (cap < 0) {
		cap = 0;
...
static	void R_LoadLightmaps( lump_t *l ) {
	...
	for ( i = 0 ; i < tr.numLightmaps ; i++ ) {
		...
		tr.lightmaps[i] = R_CreateImage( va("*lightmap%d",i), image, 
			LIGHTMAP_SIZE, LIGHTMAP_SIZE, qfalse, qfalse, r_glClampToEdge->integer ? GL_CLAMP_TO_EDGE : GL_CLAMP );
	}
...
static shader_t *ShaderForShaderNum( int shaderNum, int lightmapNum ) {
	...
	shader = R_FindShader( dsh->shader, lightmapNum, qtrue, qfalse );
...
static	void R_LoadFogs( lump_t *l, lump_t *brushesLump, lump_t *sidesLump ) {
	...
	for ( i=0 ; i<count ; i++, fogs++) {
		...
		shader = R_FindShader( fogs->shader, LIGHTMAP_NONE, qtrue, qfalse );
