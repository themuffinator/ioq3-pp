...
void RE_ScaleCorrection( float *x, float *y, float *w, float *h, char *shaderName ) {
	...
	if ( !r_arc_uiMode->integer ) return;
	else {
...
void RE_StretchAspectPic( float x, float y, float w, float h, 
	...
	// detect crosshairs and fix aspect
		...
		if ( Q_stricmp(Cvar_VariableString("fs_game"), "edawn")
				&& Q_stricmp(Cvar_VariableString("fs_game"), "excessiveplus") ) {
	...
	// uniform fit
	else if ( (w < glConfig.vidWidth && h < glConfig.vidHeight) && !strstr(cmd->shader->name, "levelshots") ) {
		if (/* (w == glConfig.vidWidth && h == glConfig.vidHeight) && */r_arc_uiMode->integer == 1 ) {
			...
			RE_ScaleCorrection( &x, &y, &w, &h, cmd->shader->name );
		}
		// uniform fill
