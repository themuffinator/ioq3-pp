...
//ql
...

	{
		...
		Q_strncpyz( filename, va("maps/%s.ent", (sv.do_ent_or && sv.ent_or) ? sv.ent_or : sv_mapname->string), sizeof(filename) );
		...
		if ( FS_FileExists(filename) ) {
			...
			if ( f.c ) {
				...
				//Com_Printf( "Overridden entity string from file (%s).\n", filename );
	...
	if ( !entor ) {
		...
		if ( sv_dumpEntities->integer ) {
			fileHandle_t	fw;
			char			filename[MAX_QPATH];

			Q_strncpyz( filename, va("%s/%s.ent", (sv_dumpEntities->integer > 1) ? "maps" : "ents", sv_mapname->string), sizeof(filename) );
			if ( !FS_FileExists(filename) ) {
				//Com_Printf( "Attempting to write entity override file: %s...\n", filename );
				fw = FS_FOpenFileWrite( filename );

				if ( fw ) {
					char *wstr;
					FS_Write( wstr, strlen(wstr), fw );

					sv.entityParsePoint = strstr( wstr, "{" );
