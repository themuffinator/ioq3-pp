...
//nql
	qboolean	cappedx = qfalse, cappedy = qfalse;
	float		mx, my, capx = 0.0f, capy = 0.0f;
//-nql
...
//nql
	// determine caps
	if ( cl_mouseSensCap->value > 0.0f ) {
		capx = mx * cl_mouseSensCap->value;
		capy = my * cl_mouseSensCap->value;
	}
//-nql
	
	if (cl_mouseAccel->value != 0.0f) {
//nql
		float frame_power = (float)frame_msec;

		if ( cl_mouseAccelPower->integer != 2 ) {
			frame_power *= 2;
			frame_power /= cl_mouseAccelPower->value;
		}
//-nql
		if(cl_mouseAccelStyle->integer == 0) {
			...
			float rate;
//nql			
			rate = sqrt(mx * mx + my * my) / frame_power;
//-nql
...
//nql
			rate[0] = fabs(mx) / frame_power;
			rate[1] = fabs(my) / frame_power;
//-nql
...
//nql
	...
	if ( cl_mouseSensCap->value > 0.0f ) {
		if ( capx ) {
			if ( fabs(mx) > fabs(capx) ) {
				mx = capx;
				cappedx = qtrue;
			}
		}
		if ( capy ) {
			if ( fabs(my) > fabs(capy) ) {
				my = capy;
				cappedy = qtrue;
			}
		}
	...
	if ( cl_debugMouse->integer ) Com_Printf( "mx: %f%s, my: %f%s\n", mx, cappedx ? "(*)" : "", my, cappedy ? "(*)" : "" );
//-nql
