...
static sfx_t *S_FindName( const char *name ) {
	...
	if (!name[0]) {
		Com_VPrintf( S_COLOR_YELLOW "S_FindName: Sound name is empty\n" );
		return NULL;
	}

	if (strlen(name) >= MAX_QPATH) {
		Com_VPrintf( S_COLOR_YELLOW "S_FindName: Sound name is too long: %s\n", name );
		return NULL;
	}
...
====================
*/
static void S_Base_StartSoundEx( vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfxHandle, qboolean localSound, const qboolean cgame ) {
	channel_t	*ch;
	sfx_t		*sfx;
...
====================
*/
void S_Base_StartSound( vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfxHandle, const qboolean cgame ) {
	S_Base_StartSoundEx( origin, entityNum, entchannel, sfxHandle, qfalse, cgame );
}
...
==================
*/
void S_Base_StartLocalSound( sfxHandle_t sfxHandle, int channelNum, const qboolean cgame ) {
	if ( !s_soundStarted || s_soundMuted ) {
		return;
	}
	...
	S_Base_StartSoundEx( NULL, listener_number, channelNum, sfxHandle, qtrue, cgame );
}
...
======================
*/
static void S_OpenBackgroundStream( const char *filename, const qboolean cgame ) {
	...
	s_backgroundStream->info.menu = !cgame;
	if(!s_backgroundStream) {
...
======================
*/
void S_Base_StartBackgroundTrack( const char *intro, const char *loop, const qboolean cgame ) {
	if ( !intro ) {
		intro = "";
	}
	if ( !loop || !loop[0] ) {
	...
	S_OpenBackgroundStream( intro, cgame );
}
...
void S_UpdateBackgroundTrack( void ) {
	...
	if ( !s_backgroundStream->info.menu && s_musicVolume->value <= 0 ) {
		return;
	}
	if ( s_backgroundStream->info.menu && s_menuMusicVolume->value <= 0 ) {
		return;
	}
	...
	while ( s_rawend[0] < s_soundtime + MAX_RAW_SAMPLES ) {
		...
		else
		{
			...
			{
				S_OpenBackgroundStream( s_backgroundLoop, !s_backgroundStream->info.menu );
				if(!s_backgroundStream)
					return;
			}
			else
