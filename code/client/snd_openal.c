...
static sfxHandle_t S_AL_BufferFind(const char *filename)
{
	...
	if ( !filename[0] ) {
		Com_VPrintf( S_COLOR_YELLOW "S_AL_BufferFind: Sound name is empty\n" );
		return 0;
	}

	if ( strlen( filename ) >= MAX_QPATH ) {
		Com_VPrintf( S_COLOR_YELLOW "S_AL_BufferFind: Sound name is too long: %s\n", filename );
		return 0;
	}

	if ( filename[0] == '*' ) {
		Com_VPrintf( S_COLOR_YELLOW "S_AL_BufferFind: Tried to load player sound directly: %s\n", filename );
		return 0;
	}
...
static void S_AL_BufferUseDefault(sfxHandle_t sfx)
{
	...
	Com_VPrintf( S_COLOR_YELLOW "WARNING: Using default sound for %s\n", knownSfx[sfx].filename);
...
static void S_AL_BufferUnload(sfxHandle_t sfx)
{
	...
	if(qalGetError() != AL_NO_ERROR)
		Com_VPrintf( S_COLOR_RED "WARNING: Can't delete sound buffer for %s\n",
				knownSfx[sfx].filename);
...
qboolean S_AL_SrcInit( void )
{
	...
	Com_VPrintf( "Allocated %d sources.\n", srcCount);
...
static void S_AL_SaveLoopPos(src_t *dest, ALuint alSource)
{
	...
	if((error = qalGetError()) != AL_NO_ERROR)
	{
		...
		{
			Com_VPrintf(S_COLOR_YELLOW "WARNING: Could not get time offset for alSource %d: %s\n",
...
static qboolean S_AL_CheckInput(int entityNum, sfxHandle_t sfx)
{
	...
	if (sfx < 0 || sfx >= numSfx)
	{
		Com_VPrintf(S_COLOR_RED "ERROR: S_AL_CheckInput: handle %i out of range\n", sfx);
		return qtrue;
	}
...
*/
static
void S_AL_StartLocalSound( sfxHandle_t sfx, int channel, const qboolean cgame ) {
	srcHandle_t src;
...
=================
*/
static void S_AL_StartSound( vec3_t origin, int entnum, int entchannel, sfxHandle_t sfx, const qboolean cgame )
{
	vec3_t sorigin;
	srcHandle_t src;
	src_t *curSource;
	...
	else
	{
		...
		{
			S_AL_StartLocalSound( sfx, entchannel, qtrue );
			return;
		}
					...
					if(curSource->priority == SRCPRI_AMBIENT)
					{
						...
						if(curSfx->loopActiveCnt)
						{
							...
							if((error = qalGetError()) != AL_NO_ERROR)
							{
								if(error != AL_INVALID_ENUM)
								{
									Com_VPrintf(S_COLOR_YELLOW "WARNING: Cannot get sample offset from source %d: "
...
void S_AL_RawSamples(int stream, int samples, int rate, int width, int channels, const byte *data, float volume, int entityNum)
{
	...
	if(streamSourceHandles[stream] == -1)
	{
		...
		if(streamSourceHandles[stream] == -1)
		{
			Com_VPrintf( S_COLOR_RED "ERROR: Can't allocate streaming streamSource\n");
			return;
		}
	}
...
void S_AL_MusicProcess(ALuint b)
{
	...
	{
		S_AL_StopBackgroundTrack( );
		Com_VPrintf( S_COLOR_RED "ERROR: while buffering data for music stream - %s\n",
				S_AL_ErrorMsg( error ) );
		return;
	}
}
...
=================
*/
static void S_AL_StartBackgroundTrack( const char *intro, const char *loop, const qboolean cgame ) {
	int i;
	qboolean issame;
	...
	S_AL_Gain( musicSource, s_alGain->value * (cgame ? s_musicVolume->value : s_menuMusicVolume->value) );
...
void S_AL_Shutdown( void )
{
	...
	if (alCaptureDevice != NULL) {
		...
		Com_VPrintf( "OpenAL capture device closed.\n" );
	}
#endif
