...
//nql
//qboolean	chat_team;
int			chat_type;		// 1 = say, 2 = say_team, 3 = tell_target, 4 = tell_attacker
//-nql
...
===================
*/
static void Field_VariableSizeDraw( field_t *edit, const float x, const float y, const float width, const int size,
								   const qboolean showCursor, const qboolean noColorEscape, const float scale, const float *color ) {
	int		len;
	int		drawLen;
	int		prestep;
	int		cursorChar;
	...
	// draw it
	SCR_DrawScaledString( x, y, str, color, qfalse, noColorEscape, scale, qfalse );
	...
	if ( showCursor ) {
		...
		lcursor = drawLen - strlen( str );

		SCR_DrawScaledChar( x + (edit->cursor - prestep - lcursor) * SMALLCHAR_WIDTH*scale, y, cursorChar, scale, qfalse );
	}
}
...
void Field_Draw( field_t *edit, const float x, const float y, const float w, const qboolean showCursor, const qboolean noColorEscape, const float scale, const float *color ) {
	Field_VariableSizeDraw( edit, x, y, w, SMALLCHAR_WIDTH, showCursor, noColorEscape, scale, color );
}
...
void Field_KeyDownEvent( field_t *edit, int key ) {
	...
	switch ( key ) {
		...
		case K_RIGHTARROW:
			if ( edit->cursor < len ) {
				// +ctrl skips to the end of the word
				if ( keys[K_CTRL].down ) {
					int i;
					for ( i = edit->cursor ; i < len ; i++ ) {
						if ( edit->buffer[i] == ' ' ) continue;
						if ( edit->buffer[i] == ';' ) continue;
						break;
					}
					for ( ; i < len-1 ; i++ ) {
						if ( edit->buffer[i+1] == ' ' ) break;
						if ( edit->buffer[i+1] == ';' ) break;
						if ( edit->buffer[i+1] == '\\' ) break;
						if ( edit->buffer[i+1] == '/' ) break;
					}
					edit->cursor = ( i+1 > len ) ? len : i+1;
				} else {
					edit->cursor++;
				}
			}
		...
		case K_LEFTARROW:
			if ( edit->cursor > 0 ) {
				...
				if ( keys[K_CTRL].down ) {
					int i;
					for ( i = edit->cursor ; i > 0 ; i-- ) {
						if ( edit->buffer[i-1] == ' ' ) continue;
						if ( edit->buffer[i-1] == ';' ) continue;
						break;
					}
					for ( ; i > 0 ; i-- ) {
...
//muff
	// ctrl-D performs a condump
	if ( key == 'd' && keys[K_CTRL].down ) {
		Cbuf_AddText( "condump console\n" );
		return;
	}

	// ctrl-F initiates the find command
	if ( key == 'f' && keys[K_CTRL].down ) {
		Con_ClearField( qfalse );
		Q_strncpyz( g_consoleField.buffer, "\\find ", sizeof(g_consoleField.buffer) );
		g_consoleField.cursor += strlen( "\\find " );
		return;
	}

	// ctrl-minus reduces console font size
	if ( (key == '-' || key == K_KP_MINUS) && keys[K_CTRL].down ) {
		Cbuf_AddText( "cvarAdd con_scale -0.1 0.2 1.0\n" );
		return;
	}

	// ctrl-minus enlarges console font size
	if ( (key == '+' || key == K_KP_PLUS) && keys[K_CTRL].down ) {
		Cbuf_AddText( "cvarAdd con_scale 0.1 0.2 1.0\n" );
		return;
	}
//-muff
...
//nql
			if ( chat_playerNum != -1 ) {
				if ( chat_type == CHATT_TARGET ) {
					Com_sprintf( buffer, sizeof(buffer), "tell_target %i \"%s\"\n", chat_playerNum, chatField.buffer );
				} else {
					Com_sprintf( buffer, sizeof(buffer), "tell_attacker %i \"%s\"\n", chat_playerNum, chatField.buffer );
				}
			} else {
				if ( chat_type == CHATT_NORMAL ) {
					Com_sprintf( buffer, sizeof(buffer), "say \"%s\"\n", chatField.buffer );
				} else if (chat_type == CHATT_TEAM ) {
...
void CL_KeyDownEvent( int key, unsigned time )
{
	...
	if ( keys[K_ALT].down && (key == K_ENTER || key == K_KP_ENTER) ) {
