...
=================
*/
void S_StartSound( vec3_t origin, int entnum, int entchannel, sfxHandle_t sfx, const qboolean cgame ) {
	if( si.StartSound ) {
		si.StartSound( origin, entnum, entchannel, sfx, cgame );
	}
}
...
=================
*/
void S_StartLocalSound( sfxHandle_t sfx, int channelNum, const qboolean cgame ) {
	if ( si.StartLocalSound ) {
		si.StartLocalSound( sfx, channelNum, cgame );
	}
}
...
=================
*/
void S_StartBackgroundTrack( const char *intro, const char *loop, const qboolean cgame ) {
	if ( si.StartBackgroundTrack ) {
		si.StartBackgroundTrack( intro, loop, cgame );
	}
}
...
void S_Play_f( void ) {
	...
	for( i = 1; i < c; i++ ) {
		...
		if( h ) {
			si.StartLocalSound( h, CHAN_LOCAL_SOUND, qfalse );
		}
	}
}
...
void S_Music_f( void ) {
	...
	if ( c == 2 ) {
		si.StartBackgroundTrack( Cmd_Argv(1), NULL, qtrue );
	} else if ( c == 3 ) {
		si.StartBackgroundTrack( Cmd_Argv(1), Cmd_Argv(2), qtrue );
	} else {
