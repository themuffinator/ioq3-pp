...
cvar_t	*cl_currentServerAddress;
cvar_t	*cl_debugMouse;
...
void CL_Voip_f( void )
{
	...
	if (strcmp(cmd, "ignore") == 0) {
		...
		if (Cmd_Argc() > 3) {
			...
			Com_xPrintf( "Usage", "voip gain", "<playerID#> [value]" );
		}
		...
		Com_xPrintf( "Usage", "voip [un]ignore", "<playerID#>" );
		Com_xPrintf( "", "     voip [un]muteall", "" );
		Com_xPrintf( "", "     voip gain", "[value]" );
	}
}
...
void CL_Connect_f( void ) {
	...
	if ( argc != 2 && argc != 3 ) {
		Com_xPrintf( "Usage", "connect", "[-4|-6] [server]" );
		return;	
	}
...
void CL_InitRenderer( void ) {
	...
	cls.consoleShader = re.RegisterShaderExt( "console", qfalse, qtrue );
...
void CL_InitRef( void ) {
	...
	Com_VPrintf( "----- Initializing Renderer ----\n" );	//Com_VPrintf
...
static void CL_GenerateQKey(void)
{
	...
	FS_FCloseFile( f );
	if( len == QKEY_SIZE ) {
		Com_VPrintf( "QKEY found.\n" );	//Com_VPrintf
		return;
	}
	else {
		if( len > 0 ) {
			Com_VPrintf( "QKEY file size != %d, regenerating\n", QKEY_SIZE );	//Com_VPrintf
		}

		Com_VPrintf( "QKEY building random string\n" );	//Com_VPrintf
...
*/
void CL_Init( void ) {
	Com_VPrintf( "----- Client Initialization -----\n" );	//Com_VPrintf
...
// ql cvars
	...
	Cvar_CheckRange( cl_allowConsoleChat, 0, 1, qtrue );
	cl_currentServerAddress = Cvar_Get( "cl_currentServerAddress", "", CVAR_ROM );
	cl_debugMouse = Cvar_Get( "cl_debugMouse", "0", 0 );
	Cvar_CheckRange( cl_debugMouse, 0, 1, qtrue );
	cl_demoRecordMessage = Cvar_Get( "cl_demoRecordMessage", "1", CVAR_ARCHIVE );
	Cvar_CheckRange( cl_demoRecordMessage, 0, 1, qtrue );
	cl_mouseAccelPower = Cvar_Get( "cl_mouseAccelPower", "2", CVAR_ARCHIVE );
	cl_mouseSensCap = Cvar_Get( "cl_mouseSensCap", "0", CVAR_ARCHIVE );
	cl_quitOnDemoCompleted = Cvar_Get( "cl_quitOnDemoCompleted", "0", CVAR_ARCHIVE );
	Cvar_CheckRange( cl_quitOnDemoCompleted, 0, 1, qtrue );
// -ql
...
//	Cbuf_Execute ();
	...
	Com_VPrintf( "----- Client Initialization Complete -----\n" );	//Com_VPrintf
}
...
void CL_GlobalServers_f( void ) {
	...
	if ((count = Cmd_Argc()) < 3 || (masterNum = atoi(Cmd_Argv(1))) < 0 || masterNum > MAX_MASTER_SERVERS - 1)
	{
		...
		Com_xPrintf( "Usage", "globalservers", "<master# 0-%d> <protocol> [keywords]", MAX_MASTER_SERVERS - 1 );
		return;	
	}
...
void CL_Ping_f( void ) {
	...
	if ( argc != 2 && argc != 3 ) {
		Com_xPrintf( "Usage", "ping", "[-4|-6] [server]" );
		return;	
	}
...
void CL_ServerStatus_f(void) {
	...
	if ( argc != 2 && argc != 3 )
	{
		...
		{
			Com_Printf( "Not connected to a server.\n" );
			Com_xPrintf( "Usage", "serverstatus", "[-4|-6] [server]" );
			return;
		}
