...
//nql
	fs_verbose = Cvar_Get( "fs_verbose", "0", CVAR_ARCHIVE );
//-nql

	if ( fs_verbose->integer )
...
//nql
	fs_useBaseHome = Cvar_Get( "fs_useBaseHome", "1", CVAR_INIT );
//-nql

	homePath = Sys_DefaultHomePath();
//nql
	//if (!homePath || !homePath[0]) {
	if ( fs_useBaseHome->integer || !homePath || !homePath[0] ) {
//-nql
...
void FS_Restart( int checksumFeed ) {
	...
	if ( Q_stricmp(fs_gamedirvar->string, lastValidGame) ) {
		...
		if ( !Com_SafeMode() ) {
			Cbuf_AddText( "execc " DEFCONFIG_CFG "\n" );
		}
	}
