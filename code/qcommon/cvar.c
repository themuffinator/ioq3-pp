...
//muff
	//const qboolean integral = ( Q_isintegral(v->min) && Q_isintegral(v->max) ) ? qtrue : qfalse;
	const qboolean integral = v->integral ? qtrue : qfalse;
//-muff
	
	Com_Printf( "\"%s\" is:\"" S_COLOR_DIRTYWH "%s" S_COLOR_WHITE "\"", v->name, v->string );

	if ( !(v->flags & CVAR_ROM) ) {
		if ( !Q_stricmp(v->string, v->resetString) ) {
			Com_Printf( S_COLOR_DIRTYWH " (default)" );
		} else {
			Com_Printf( S_COLOR_GREY70 " default:\"" S_COLOR_DIRTYWH "%s" S_COLOR_GREY70 "\"", v->resetString );
		}

//muff
		if ( integral ) {
			if ( v->min || v->max ) {
				Com_Printf( S_COLOR_GREY70 " min:\"" S_COLOR_DIRTYWH "%i" S_COLOR_GREY70 "\"", (int)v->min );
				Com_Printf( S_COLOR_GREY70 " max:\"" S_COLOR_DIRTYWH "%i" S_COLOR_GREY70 "\"", (int)v->max );
			}
		} else {
			if ( v->min || v->max ) {
				Com_Printf( S_COLOR_GREY70 " min:\"" S_COLOR_DIRTYWH "%f" S_COLOR_GREY70 "\"", v->min );
				Com_Printf( S_COLOR_GREY70 " max:\"" S_COLOR_DIRTYWH "%f" S_COLOR_GREY70 "\"", v->max );
			}
		}
	}
	...
	if ( v->latchedString ) {
		Com_Printf( "\n" );
		Com_Printf( S_COLOR_GREY70 "latched: \"" S_COLOR_DIRTYWH "%s" S_COLOR_GREY70 "\"\n", v->latchedString );
	}
/*
	if ( v->description ) {
		Com_Printf( "\n" );
		Com_Printf( S_COLOR_GREY70 " description:\"" S_COLOR_DIRTYWH "%s" S_COLOR_GREY70 "\"", v->description );
	}
*/
	Com_Printf( "\n" );
//-muff
...
void Cvar_CmdCvarClear_f( void ) {
	...
	if ( Cmd_Argc() != 2 ) {
		Com_xPrintf( "Usage", "cvarClear", "<variable>" );
		return;
	}

	Cvar_Set2( Cmd_Argv(1), "", qfalse );
}
//-qzero

/*
============
Cvar_CmdCycle_f - ydnar

Cycles a cvar for easy single key binding
============
*/
void Cvar_CmdIntCycle_f( void ) {
	int start, end, step, oldvalue, value;

	if ( Cmd_Argc() < 4 || Cmd_Argc() > 5 ) {
		Com_xPrintf( "Usage", "cvarIntCycle", "<variable> <start> <end> [step]" );
		return;
	}
...
void Cvar_CmdCycle_f( void ) {
	float start, end, step, oldvalue, value;

	if ( Cmd_Argc() < 4 || Cmd_Argc() > 5 ) {
		Com_xPrintf( "Usage", "cvarCycle", "<variable> <start> <end> [step]" );
		return;
	}
...
void Cvar_Print_f( void ) {
	...
	if ( Cmd_Argc() != 2 ) {
		Com_xPrintf( "Usage", "cvarPrint", "<variable>" );
		return;
	}
...
void Cvar_Toggle_f( void ) {
	...
	if(c < 2) {
		Com_xPrintf( "Usage", "toggle", "<variable> [value1, value2, ...]" );
		return;
	}
...
void Cvar_Set_f( void ) {
	...
	if ( c < 2 ) {
		Com_xPrintf( "Usage", "%s", "<variable> <value>", cmd );
		return;
	}
	if ( c == 2 ) {
		Cvar_Print_f();
...
void Cvar_Reset_f( void ) {
	if ( Cmd_Argc() != 2 ) {
		Com_xPrintf( "Usage", "reset", "<variable>" );
		return;
	}
...
void Cvar_List_f( void ) {
	...
	for ( var = cvar_vars ; var ; var = var->next, i++ ) {
		if ( !var->name || (match && !Com_Filter(match, var->name, qfalse)) )
			continue;
		colorString = Q_stricmp(var->string, var->resetString) ? S_COLOR_ALPHA_ORANGERED : S_COLOR_DIRTYWH;
		...
		Com_Printf( " %s%s", colorString, var->name );
		Com_Printf( S_COLOR_GREY70 " \"" S_COLOR_WHITE "%s" S_COLOR_GREY70 "\"\n", var->string);
	}
	Com_Printf( S_COLOR_GREY70 "\n--------------------------------\n" );

	Com_Printf( S_COLOR_WHITE "%i " S_COLOR_GREY70 "total cvars\n", i );
	Com_Printf( S_COLOR_WHITE "%i " S_COLOR_GREY70 "cvar indexes\n\n", cvar_numIndexes );
}
...
============
*/
void Cvar_Restart( const qboolean unsetVM, const qboolean onlyCleanUp ) {
	cvar_t	*curvar;

	curvar = cvar_vars;

	while ( curvar ) {
		if ( (curvar->flags & (CVAR_USER_CREATED|CVAR_NONEXISTENT) ) ||
		...
		if ( !onlyCleanUp && !(curvar->flags & (CVAR_ROM | CVAR_INIT | CVAR_NORESTART)) ) {
...
============
*/
void Cvar_CleanUp_f( void ) {
	Cvar_Restart( qfalse, qtrue );
	Com_ExecuteCfg();
}
...
*/
/*
void Cvar_Description( cvar_t *var, char *description ) {
...
void Cvar_Init (void)
{
	...
	Cmd_AddCommand( "cvarPrint", Cvar_Print_f );	//muff: renamed
...
//nql
	Cmd_AddCommand( "cvar_cleanup", Cvar_CleanUp_f );
...
//-nql
	Cmd_AddCommand( "cvarIntCycle", Cvar_CmdIntCycle_f );  // ydnar
	Cmd_SetCommandCompletionFunc( "cvarIntCycle", Cvar_CompleteCvarName );
	Cmd_AddCommand( "cvarCycle", Cvar_CmdCycle_f );  // ydnar
	Cmd_SetCommandCompletionFunc( "cvarCycle", Cvar_CompleteCvarName );
}
