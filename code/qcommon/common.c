...
/*
static qboolean Com_OpenLogFile( const char *name, char *msg, const qboolean deflog, qboolean opening_logfile ) {
	// logfile
	if ( com_logfile && com_logfile->integer ) {
    // TTimo: only open the qconsole.log if the filesystem is in an initialized state
    //   also, avoid recursing in the qconsole.log opening (i.e. if fs_debug is on)
		if ( !logfile && FS_Initialized() && !opening_logfile ) {
			struct tm *newtime;
			time_t aclock;

			if ( !name ) {
				Com_OpenLogFile( "qconsole.log", msg, qtrue, opening_logfile );
				return qfalse;
			}

			opening_logfile = qtrue;
			...
			if(logfile)
			{
				...
				if ( deflog ) {
					Com_Printf( "Opening default logfile (%s) failed!\n", name );
					Cvar_SetValue( "logfile", 0 );
				} else {
					Com_Printf( "Opening logfile (%s) failed, reverting to default!\n", name );
					Com_OpenLogFile( "qconsole.log", msg, qtrue, opening_logfile );
					return qfalse;
				}
			}
			opening_logfile = qfalse;
		}
		if ( logfile && FS_Initialized()) {
			FS_Write(msg, strlen(msg), logfile);
		}
	}
	return opening_logfile;
}
*/
...
void QDECL Com_Printf( const char *fmt, ... ) {
	...
	static		qboolean opening_logfile = qfalse;
	...
	// logfile
	//opening_logfile = Com_OpenLogFile( com_logfilename->string, msg, qfalse, opening_logfile );
	// logfile
	if ( com_logfile && com_logfile->integer ) {
    // TTimo: only open the qconsole.log if the filesystem is in an initialized state
    //   also, avoid recursing in the qconsole.log opening (i.e. if fs_debug is on)
		if ( !logfile && FS_Initialized() && !opening_logfile ) {
			...
			if(logfile)
			{
				Com_Printf( "Opened logfile " S_COLOR_DIRTYWH "%s" S_COLOR_WHITE " on " S_COLOR_DIRTYWH "%s" S_COLOR_WHITE "\n", filename, asctime(newtime) );
				...
				Com_Printf( "Failed to open logfile " S_COLOR_DIRTYWH "%s" S_COLOR_WHITE "\n", filename );
				Cvar_SetValue( "logfile", 0 );
			}
			opening_logfile = qfalse;
		}
...
void QDECL Com_xPrintf( const char *header, const char *mid, char *last, ... ) {
	...
	last = va( S_COLOR_DIRTYWH "> %s: " S_COLOR_WHITE "%s " S_COLOR_GREY70 "%s\n" S_COLOR_WHITE, header, mid, last );
/*
	sheader = header ? va(S_COLOR_DIRTYWH "> %s: ", header) : "";
	smid = mid ? va("%s" S_COLOR_WHITE "%s ", (header ? "" : S_COLOR_DIRTYWH "> "), mid) : "";
	slast = last ? va(S_COLOR_GREY70 "%s", last) : "";
*/
	/*
	Com_sprintf( stxt, sizeof(stxt), "%s%s%s",
		(header ? va(S_COLOR_DIRTYWH "> %s: ", header) : ""),
		(mid ? va("%s" S_COLOR_WHITE "%s ", (header ? "" : S_COLOR_DIRTYWH "> "), mid) : ""),
		(last ? va(S_COLOR_GREY70 "%s", last) : "")
		);
		*/
/*
	Com_Printf( "Com_Printf(): %s\n", sheader );
	Com_Printf( "Com_Printf(): %s\n", smid );
	Com_Printf( "Com_Printf(): %s\n", slast );
*/	
	//last = stxt;	//va( "%s\n" S_COLOR_WHITE, stxt
...
Verbose printing: A Com_Printf that only shows up if the "verbose" cvar is set
================
*/
void QDECL Com_VPrintf( const char *fmt, ...) {
	va_list		argptr;
	char		msg[MAXPRINTMSG];
		
	if ( !com_verbose->integer ) {
		return;			// don't spam people shitless with routine print-outs
	}
...
void Com_TouchMemory( void ) {
	...
	Com_VPrintf( "Com_TouchMemory: %i msec\n", end - start );
}
...
void Com_ExecuteCfg( void ) {
	...
	if(!Com_SafeMode())
	{
		...
		Cbuf_ExecuteText( EXEC_NOW, "execc " DEFCONFIG_CFG "\n" );
		Cbuf_Execute();
		Cbuf_ExecuteText( EXEC_NOW, "exec " AUTOEXEC_CFG "\n" );
		Cbuf_Execute();
	}
}
...
static void Com_WriteCDKey( const char *filename, const char *ikey ) {
	...
	FS_Printf( f, "\n// This file is auto-generated by %s, do not modify.\r\n", PRODUCT_NAME );
...
//muff
	com_logfilename = Cvar_Get( "com_logfilename", "logs/console.log", CVAR_ARCHIVE );
	com_playLogoCine = Cvar_Get( "com_playLogoCine", "0", CVAR_ARCHIVE );
	com_verbose = Cvar_Get( "verbose", "0", 0 );
//-muff
	...
	Com_VPrintf( "--- Common Initialization Complete ---\n" );	//Com_VPrintf
	if ( !com_verbose->integer )
		Com_Printf( "--------------------------------\n" );
}
...
void Com_WriteConfigToFile( const char *filename ) {
	...
	FS_Printf( f, "// This configuration file is generated by %s, do not modify.\r\n", PRODUCT_NAME );
	Cmd_WriteAlias( f );
	Key_WriteBindings( f );
...
void Com_WriteConfiguration( void ) {
	...
	if ( !(cvar_modifiedFlags & CVAR_ARCHIVE ) && !alias_modified ) {
		return;
	}
	cvar_modifiedFlags &= ~CVAR_ARCHIVE;
	alias_modified = qfalse;

	Com_WriteConfigToFile( CFGPATH DEFCONFIG_CFG );
...
void Com_WriteConfig_f( void ) {
	...
	char	*path = Cmd_Argv(1), *ss;
	int		ilen, isub;

	if ( Cmd_Argc() != 2 ) {
		Com_xPrintf( "Usage", "writeconfig", "<filename>" );
		Com_xPrintf( "Description", "Writes current settings to a file in your cfg folder, assumes .cfg as default file extension", "" );
		return;
	}
	
	//Com_Printf( "Com_WriteConfig_f(): arg1:%s\n", Cmd_Argv(1) );

	ss = strstr(Cmd_Argv(1), CFGPATH);
	//Com_Printf( "Com_WriteConfig_f(): strstr:%s\n", ss );
	isub = ss ? strlen(ss) : 0;
	ilen = strlen( Cmd_Argv(1) );

	if ( (ilen - isub) != 0 ) {
		Com_sprintf( filename, sizeof(filename), "%s%s", CFGPATH, path );
		
	} else
		Q_strncpyz( filename, path, sizeof(filename) );

	COM_DefaultExtension( filename, sizeof(filename), ".cfg" );
	Com_Printf( S_COLOR_GREY70 "Writing " S_COLOR_DIRTYWH "%s" S_COLOR_WHITE "\n", filename );
